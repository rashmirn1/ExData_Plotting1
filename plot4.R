...
rpower <- tab$Global_reactive_power
rp1 <- rpower[cond1]
rp2 <- rpower[cond2]
rp <- c(rp1,rp2)
...
plot(dttmf,p, type ="l",xlab = "",ylab = "Global Active Power",xaxt="n")
axis(1,xat,xlb)
...
colnm <- names(tab)
lgd <- colnm[7:9]
legend('topright', lgd, lty=1, col=c('black','red', 'blue'),bty='n',cex=.75)
axis(1,xat,xlb)

plot(dttmf,volt, type ="l",xlab = "datetime",ylab = "Voltage",xaxt="n")
axis(1,xat,xlb)

plot(dttmf,rp, type ="l",xlab = "datetime",ylab = "Global_reactive_power",xaxt="n")
axis(1,xat,xlb)
